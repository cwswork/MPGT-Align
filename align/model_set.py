...
from align.model_nag import TransformerModel, drop_nei_feature
...
class align_set():
    def __init__(self, myconfig):
        ...
        self.Out_Dir = myconfig.Out_Dir
        self.best_mode_pkl_title = myconfig.Out_Dir + myconfig.time_str
        #  Load KGs data
        self.kgs_data = pre_loadKGs.load_KGs_data(myconfig)

        myconfig.input_dim = self.kgs_data.input_dim
        ## Hyper Parameter ######################

        ## Model and optimizer ######################
        self.graph_model = TransformerModel(myconfig).to(myconfig.device) #
        self._graph_model = TransformerModel(myconfig).to(myconfig.device)
        self._graph_model.update(self.graph_model)

        self.optimizer = torch.optim.AdamW(self.graph_model.parameters(), lr=myconfig.peak_lr, weight_decay=myconfig.weight_decay)  # 权重衰减（参数L2损失）weight_decay =5e-4
    ...
    def model_run(self, beg_epochs=0):
        ...
        last_epoch = 0
        # initial（SF） accuracy
        kg1_name_embed, kg2_name_embed = self.kgs_data.kg_embed
        ...
        for epochs_i in range(beg_epochs, self.myconfig.train_epochs):
            ...
            if (epochs_i>=self.myconfig.start_valid) and (epochs_i % self.myconfig.val_freq == 0):
        ...
        # Last Test
        self.runTest(epochs_i=last_epoch)  #  Testing
        # # Best Test Load model
        self.myconfig.myprint('\nBest epoch-{:04d}:'.format(self.best_epochs))
        #if last_epoch != self.best_epochs:
        best_savefile = '{}-{}.pkl'.format(self.best_mode_pkl_title, 'Best')
        ...
        oldfilename = self.Out_Dir[:-1]
        newfilename = oldfilename + '-' + str(self.best_test_hits1)
        return re, oldfilename, newfilename
    ...
    def runTrain(self, epochs_i):
        ...
        for (batch_embed0) in self.kgs_data.train_set: # tqdm
            batch1 = drop_nei_feature(batch_embed0, self.myconfig.adj_dropout)  # adj_dropout 取的是0.2，这里值越大的话破坏程度越大
            batch2 = drop_nei_feature(batch_embed0, 0.3)

            self.optimizer.zero_grad()  # 梯度清零
            ent_out1 = self.graph_model.forward(batch1)
            with torch.no_grad():
                self._graph_model.eval()
                ent_out2 = self._graph_model.forward(batch2)
    ...
    def runValid(self, epochs_i):
        ...
        with torch.no_grad():  # 1
            ...
            # 3 model action
            for batch_id, (batch_embed0) in enumerate(self.kgs_data.val_set):
                ## 丢邻居ff
                batch_embed = self.graph_model.forward(batch_embed0)
    ...
    def runTest(self, epochs_i, isSave=False):
        ...
        with torch.no_grad():# 1
            ...
            self.graph_model.eval()
            # 3 model action
            for batch_id, (batch_embed0) in enumerate(self.kgs_data.test_set):
    ...
    def save_model(self, better_epochs_i, epochs_name):
        ...
        model_savefile = '{}-{}.pkl'.format(self.best_mode_pkl_title, epochs_name)
